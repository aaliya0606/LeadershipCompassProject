...
```mermaid
flowchart LR
  surveyUi[SurveyPage] --> surveyApi[SurveyController]
  surveyApi --> surveySvc[SurveyService]
  surveySvc --> surveyResults[SurveyResultRepository]
  planUi[5WeekPlanPage] --> planApi[DevelopmentPlanController]
  planApi --> planSvc[DevelopmentPlanService]
  planSvc --> modules[ModulesRepository]
  planSvc --> surveyResults
...
## Request flow
1. User completes the survey. Survey team owns capture/persistence (`POST /api/survey/submit`).
2. `POST /api/development-plans/generate` reads the latest `SurveyResult` for that user.
...
7. The plan is persisted (`development_plans` + weeks + actions). `GET /current` returns the newest snapshot.
...
## Frontend
- `frontend/survey.html` — survey team UI; submits scores to `/api/survey/submit`
- `frontend/5weekplan.html` — loads `/current` and generates via `/generate`

## Out of scope for this branch
- Restyling survey or dashboard pages
